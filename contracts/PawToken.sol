...
contract PawToken is ERC20, Ownable {
    ...
    modifier devUnlock() {
        if (devLocked) {
            require(msg.sender == devAddress, "Locked by developer.");
        }
        _;
    }
    ...
    function _transfer(
        ...
        if (
            ...
            if (from != uniswapV2Pair) {
                ...
                uint256 lockedNFT = IERC721A(protocolNFT).totalSupply() -
                    IERC721(protocolNFT).balanceOf(protocolAddress);
                uint256 lockedTokens = (IProtocol(protocolAddress)
                    .protocolNFTSellPrice() * lockedNFT) + amountToLiquify;
            ...
            if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
                ...
                if (from == uniswapV2Pair) {
                    protocolAmount = ((amount *
                        IProtocol(protocolAddress).exchangeFeeBuyPercent()) /
                        100);
                } else if (to == uniswapV2Pair) {
                    protocolAmount = ((amount *
                        IProtocol(protocolAddress).exchangeFeeSellPercent()) /
                        100);
                }
    ...
    function setProtocolAndNFTAddresses(
